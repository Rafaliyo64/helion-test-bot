...
# ID del creador del bot: recibe por DM las recomendaciones que le hagan
CREATOR_USER_ID = int(os.getenv("CREATOR_USER_ID", "0"))

RECOMEND_TRIGGERS = [
    "recomiendale a tu creador que", "recomiéndale a tu creador que",
    "recomiendale a tu creador", "recomiéndale a tu creador",
    "dile a tu creador que", "sugierele a tu creador que",
    "sugiérele a tu creador que",
]
...
async def ask_ai(prompt: str, user_id=None, username: str | None = None,
                  mentioned_members: list[discord.Member] | None = None) -> str:
    ...
    # Recuerdos propios de quien pregunta
    if user_id is not None:
        propios = recuerdo.recordar(user_id)
        if propios:
            memoria_extra += "\n\nEsto recuerdas de charlas anteriores con esta persona:\n"
            memoria_extra += "\n".join(f"- {r}" for r in propios)

    # Si mencionó a otra persona con @, busca lo que recuerdas de ella
    personas_ya_incluidas = {user_id} if user_id is not None else set()
    if mentioned_members:
        for member in mentioned_members:
            if member.bot or member.id in personas_ya_incluidas:
                continue
            recuerdos_persona = recuerdo.recordar(member.id)
            if recuerdos_persona:
                memoria_extra += f"\n\nEsto recuerdas de {member.display_name}:\n"
                memoria_extra += "\n".join(f"- {r}" for r in recuerdos_persona)
            personas_ya_incluidas.add(member.id)

    # Si no la mencionó con @, intenta buscarla por nombre en lo que escribió
    # (solo si el mensaje parece preguntar por alguien, para no hacer ruido)
    if any(p in prompt.lower() for p in ["recuerdas", "sabes de", "que sabes"]):
        palabras = [w.strip(".,?!¿¡") for w in prompt.split() if len(w.strip(".,?!¿¡")) >= 3]
        for palabra in palabras:
            encontrados = recuerdo.buscar_por_nombre(palabra)
            for nombre_encontrado, resumenes in encontrados:
                memoria_extra += f"\n\nEsto recuerdas de {nombre_encontrado}:\n"
                memoria_extra += "\n".join(f"- {r}" for r in resumenes)

    system_prompt += memoria_extra

    reply = await ai_backend.ask(system_prompt, prompt)

    # Guarda un resumen de esta charla para el futuro
    if user_id is not None and username and reply and not reply.startswith("⚠️"):
        resumen = await ai_backend.ask(
            "Resume en una sola frase corta y neutra, en tercera persona, de "
            "qué trató este intercambio. Responde solo con la frase, nada más.",
            f"Pregunta: {prompt}\nRespuesta: {reply}",
        )
        if resumen and not resumen.startswith("⚠️"):
            recuerdo.guardar(user_id, username, resumen.strip()[:200])
...
async def on_message(message: discord.Message):
    ...
    if is_mentioned and any(t in lowered for t in RECOMEND_TRIGGERS):
        recomendacion = content
        for trig in RECOMEND_TRIGGERS:
            idx = lowered.find(trig)
            if idx != -1:
                recomendacion = content[idx + len(trig):].strip(" :,-")
                break

        if not recomendacion:
            await message.reply(
                "¿Qué quieres que le recomiende? Ej: `@Helion recomiendale a tu "
                "creador que agregue un canal de sugerencias`.",
        ...
        if not CREATOR_USER_ID:
            await message.reply(
                "⚠️ No tengo configurado quién es mi creador (falta CREATOR_USER_ID).",
                mention_author=False,
            )
            return

        try:
            creator = await bot.fetch_user(CREATOR_USER_ID)
            await creator.send(
                f"💡 **Recomendación de {message.author}** (en {message.guild.name if message.guild else 'DM'}):\n"
                f"{recomendacion}"
            )
            await message.reply("✅ Se lo hice saber a mi creador, ¡gracias por la idea!", mention_author=False)
        except discord.HTTPException:
            await message.reply("⚠️ No pude enviarle el mensaje a mi creador.", mention_author=False)
        return
    ...
    async with message.channel.typing():
        ...
        reply = await ask_ai(
            ...
            mentioned_members=other_mentions,
        )
...
async def olvidame_cmd(ctx: discord.ApplicationContext):
    recuerdo.olvidar(ctx.author.id)
    await ctx.respond("🧹 Listo, borré todo lo que recordaba de ti.", ephemeral=True)


@bot.slash_command(name="recomendar", description="Mándale una recomendación al creador del bot")
async def recomendar_cmd(
    ctx: discord.ApplicationContext,
    texto: discord.Option(str, description="Qué le quieres recomendar"),
):
    if not CREATOR_USER_ID:
        await ctx.respond("⚠️ No tengo configurado quién es mi creador (falta CREATOR_USER_ID).", ephemeral=True)
        return
    try:
        creator = await bot.fetch_user(CREATOR_USER_ID)
        await creator.send(
            f"💡 **Recomendación de {ctx.author}** (en {ctx.guild.name if ctx.guild else 'DM'}):\n{texto}"
        )
        await ctx.respond("✅ Se lo hice saber a mi creador, ¡gracias por la idea!", ephemeral=True)
    except discord.HTTPException:
        await ctx.respond("⚠️ No pude enviarle el mensaje a mi creador.", ephemeral=True)
